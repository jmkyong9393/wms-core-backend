...
def find_return_job_by_id_for_update(
    ...
    statement = select(ReturnJob).where(ReturnJob.id == job_id).with_for_update()
...
def get_return_job_or_raise(
    ...
    if job is None:
        raise ValueError(f"ReturnJob을 찾을 수 없습니다.job_id={job_id}")
...
def prepare_processing_job(
    ...
    with Session(engine) as session:
        ...
        if job.status in {
            ...
            raise ValueError(f"이미 종료된 검수 작업입니다. job_id={job_id} status={job.status}")

        if job.task_id is None:
            job.task_id = celery_task_id
        elif job.task_id != celery_task_id:
            raise ValueError(
                f"이미 다른 Celery 작업이 할당된 검수입니다. job_id={job_id} existing_task_id={job.task_id}"
            )

        image_paths = list(job.image_paths or [])

        if not image_paths:
            raise ValueError(f"검수할 이미지가 없습니다. job_id={job_id}")
...
def save_inspection_hitl_required(
    ...
    with Session(engine) as session:
        ...
        if job is None:
            raise ValueError("ReturnJob을 찾을 수 없습니다.")
...
def save_ai_inspection_result(
    ...
    with Session(engine) as session:
        ...
        if job is None:
            raise ValueError(f"ReturnJob을 찾을 수 없습니다. job_id={return_job_id}")
        ...
        if job.status != ReturnJobStatus.PROCESSING:
            raise ValueError(f"AI 결과를 저장할 수 있는 상태가 아닙니다. job_id={return_job_id} status={job.status}")

        decision = ai_result.get("decision")

        if decision not in {"APPROVE", "REJECT"}:
            raise ValueError(f"저장할 수 없는 AI 판정입니다: {decision}")
...
def save_wms_task_id(
    ...
    with Session(engine) as session:
        ...
        if job.status != ReturnJobStatus.PROCESSING:
            raise ValueError(
                f"WMS Task ID를 저장할 수 있는 상태가 아닙니다. job_id={return_job_id} status={job.status}"
            )
...
def process_saved_wms_result_with_lock(
    ...
    with Session(engine) as session:
        ...
        if job.status in {
            ...
            logger.warning(
                "이미 완료된 검수 작업입니다. WMS 재호출을 생략합니다. job_id=%s status=%s",
                return_job_id,
                job.status,
            )
            return job, False
        ...
        if job.status != ReturnJobStatus.PROCESSING:
            raise ValueError(f"WMS 처리가 가능한 상태가 아닙니다. job_id={return_job_id} status={job.status}")

        agent_logs = job.agent_logs or {}

        hitl_logs = agent_logs.get("hitl") if isinstance(agent_logs.get("hitl"), dict) else {}

        expected_wms_task_id = agent_logs.get("wms_task_id") or hitl_logs.get("wms_task_id")
    ...
    with Session(engine) as session:
        ...
        if job.status != ReturnJobStatus.PROCESSING:
            raise ValueError(f"WMS 결과를 저장할 수 없는 상태입니다. job_id={return_job_id} status={job.status}")
...
def save_inspection_failed(
    ...
    with Session(engine) as session:
        ...
        if job is None:
            logger.error(
                "FAILED 상태 저장 실패: ReturnJob을 찾을 수 없습니다. job_id=%s",
                return_job_id,
            )
            return None
        ...
        if job.status in {
            ...
            logger.warning(
                "이미 최종 처리된 작업이므로 FAILED 상태로 변경하지 않습니다. job_id=%s status=%s error=%s",
                return_job_id,
                job.status,
                error,
            )
        ...
        if job.status == ReturnJobStatus.FAILED:
            logger.warning(
                "이미 FAILED 처리된 작업이므로 중복 실패 저장을 생략합니다. job_id=%s error=%s",
                return_job_id,
                error,
            )
            return None
...
def save_wms_dispatch_failed(
    ...
    with Session(engine) as session:
        ...
        if job.status != ReturnJobStatus.PROCESSING:
            raise ValueError(
                f"WMS Task 등록 실패 정보를 저장할 수 있는 상태가 아닙니다. job_id={return_job_id} status={job.status}"
            )
        ...
        job.agent_logs = {
            **existing_logs,
            ...
            "wms_dispatch_failed_at": datetime.now(UTC).isoformat(),
...
def save_wms_processing_failed(
    ...
    with Session(engine) as session:
        ...
        if job is None:
            logger.error(
                "WMS 실패 상태 저장 실패: ReturnJob을 찾을 수 없습니다. job_id=%s",
                return_job_id,
            )
            return None
        ...
        if job.status in {
            ...
            logger.warning(
                "이미 최종 처리된 작업이므로 WMS 실패 상태로 변경하지 않습니다. job_id=%s status=%s",
                return_job_id,
                job.status,
            )
            return job
        ...
        if job.status == ReturnJobStatus.FAILED:
            logger.warning(
                "이미 FAILED 처리된 작업이므로 중복 저장을 생략합니다. job_id=%s",
                return_job_id,
            )
            return None
