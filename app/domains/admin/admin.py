...
from app.domains.admin.inbound_dashboard_service import (
    get_inbound_dashboard_summary as get_inbound_dashboard_summary_service,
)
...
def get_inspection_metrics(
    ...
    for job in jobs:
        ...
        if job.ai_inspection_completed_at is not None:
            processing_time = max(
                0.0,
                (job.ai_inspection_completed_at - job.ai_inspection_started_at).total_seconds(),
            )
...
@router.get(
    ...
    description=("최근 지정 기간의 일별 입고·출고 수량과 완료된 AI 검수 건의 평균 처리 시간을 반환합니다."),
)
...
def get_admin_inspection_history(
    ...
    fast_track: bool | None = Query(
        default=None,
        description=("Fast Track 여부 필터. true면 Fast Track 건만, false면 일반 검수 건만 조회"),
    ),
...
def get_hitl_inspection_queue(
    bucket: HITLQueueBucket = Query(
        ...
        description=("보드 구분값: PENDING, IN_REVIEW, RECHECK, COMPLETED"),
    ),
    page: int = Query(
        default=1,
        ge=1,
...
def ensure_default_fds_policies(session: Session):
    ...
    if not existing:
        for p in DEFAULT_POLICIES:
            policy = FdsPolicy(policy_key=p["policy_key"], policy_value=p["policy_value"], description=p["description"])
            session.add(policy)
        session.commit()
