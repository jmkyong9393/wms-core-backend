...
ALADIN_CATEGORY_KEYWORDS: Mapping[BookCategory, Sequence[str]] = MappingProxyType(
    {
...
def map_aladin_category(category_name: str) -> BookCategory:
    ...
    searchable_segments = category_segments[1:] if len(category_segments) > 1 else category_segments
    ...
    raise UnsupportedAladinCategoryError(f"Unsupported Aladin category: {category_name}")


def _normalize_category_path(category_name: str) -> str:
    normalized_path = "".join(category_name.split())
    if not normalized_path:
        raise UnsupportedAladinCategoryError("Aladin category name must not be empty")
