...
class OrderItemInventoryAllocation(SQLModel, table=True):
    ...
    __table_args__ = (
        UniqueConstraint(
            "order_item_id",
            "inventory_id",
            name="uq_order_item_inventory_allocation",
        ),
        CheckConstraint(
            "quantity > 0",
            name="ck_order_item_inventory_allocation_quantity_positive",
        ),
        CheckConstraint(
            "picked_quantity >= 0 AND picked_quantity <= quantity",
            name=("ck_order_item_inventory_allocation_picked_quantity"),
        ),
    )
