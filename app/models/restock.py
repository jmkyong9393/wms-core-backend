...
class OrderProposal(SQLModel, table=True):
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            ...
            name="ck_order_proposals_pending_auto_po_nonnegative",
        ),
    )
    ...
    return_job_id: uuid.UUID | None = Field(
        default=None,
        foreign_key="return_jobs.id",
        index=True,
    )
    proposal_source: RestockProposalSource = Field(
        default=RestockProposalSource.RETURN_REJECTION,
        nullable=False,
        index=True,
    )
    ...
    auto_po_order_id: uuid.UUID | None = Field(
        default=None,
