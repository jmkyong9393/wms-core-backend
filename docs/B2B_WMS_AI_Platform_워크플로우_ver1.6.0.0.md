...
flowchart TD
    subgraph "Client Layer (Vercel / Next.js)"
        A[작업자 PWA 앱 및 다수건 동시 처리 모드] -->|"1. 실시간 영상 스트리밍 - 단일 건 또는 N건 동시"| B("HTML5 Canvas 리사이징")
        F[관리자 Web] -->|"7. 리포트 로그 확인 및 승인"| G("상태 모니터링")
    end

    subgraph "API & Orchestration (AWS EKS / FastAPI & Worker)"
        C[AWS CloudFront Edge Upload] -->|"3. Redis 브로커 큐 Celery Worker 큐 INSERT 후 202 반환"| E[(AWS RDS PostgreSQL)]
        Auth{JWT RBAC Auth Guard} -->|"MASTER/WORKER 권한 검증"| E
        C -->|"클라이언트 SSE 실시간 푸시"| A
        E -->|"4. Celery/Redis 백그라운드 큐 SSE"| W[Worker Daemon]
        W -->|"5. Multi-Agent 위임"| D[LangGraph Workflow]
        D -->|"6. 리포트 로그 및 DB 반영"| E
    end
    ...
    subgraph "Analytics & LLMOps Layer"
        E -.->|"일 단위 배치 연산"| Batch[K8s CronJob report_batch.py]
        Batch -->|"리사이징 탐지 및 리포트 최적화"| F
    ...
    classDef storage fill:#FFF3E0,stroke:#FB8C00,color:#E65100,stroke-width:2px;
    classDef analytics fill:#EDE7F6,stroke:#673AB7,color:#311B92,stroke-width:2px;
    classDef auth fill:#FFEBEE,stroke:#E53935,color:#B71C1C,stroke-width:2px;

    %% =======================
    %% 클래스 적용
    %% =======================
    class A,B,F,G frontend;
    class C,E,W,D backend;
    class M1,M2 ai;
    class S3 storage;
    class Batch,LangSmith analytics;
    class Auth auth;
```
...
| :--- | :--- | :--- | :--- | :--- |
| `users` | `id` | UUID | PK | 작업자 계정 (Role: MASTER, WORKER, GUEST, PENDING) |
| `board_posts` | `id` | UUID | PK | 사내 공지 및 대시보드 게시판 (Notice, Manual, General) |
