...
uv run mypy app/                                # 타입 검사 (베이스라인 적용)
```
- 규칙셋과 ignore 사유는 `pyproject.toml`의 `[tool.ruff.lint]`에 적혀 있습니다.
  규칙을 되살리거나 새로 무시할 때는 **사유를 주석으로 남기세요.**
- mypy도 **차단 게이트**입니다. 다만 SQLModel 쿼리 표현식에 대한 오탐(`Order.shipped_at.desc()`
  같은 컬럼 연산을 파이썬 필드 타입으로 잘못 보는 문제)이 많아, 오류가 남은 44개 모듈은
  `pyproject.toml`의 `[[tool.mypy.overrides]]`에 **베이스라인으로 제외**해 두었습니다.
  - 나머지 108개 모듈은 하드 게이트로 지켜집니다. 새 파일은 처음부터 통과해야 합니다.
  - 베이스라인 목록은 **줄여 나갈 부채**입니다. 해당 파일을 손볼 때 정리하고 항목을 지우세요.
  - 반대로 깨끗한 파일에 쿼리를 추가해 오탐이 나면, 사유를 적고 목록에 추가하면 됩니다.
