...
*   **압도적인 처리 속도 (uv 및 gevent):** 무거운 처리 과정을 최신 기술(Rust 기반 uv, 초경량 비동기 스레드 gevent)로 교체하여, 동일한 서버 환경에서도 기존 대비 수십 배 빠르고 쾌적하게 트래픽을 처리합니다.
*   **클라우드 엣지 분산 통신 (Signed Cookie):** 사진을 올리고 조회할 때 중앙 서버가 뻗지 않도록, 전 세계에 퍼져 있는 클라우드 엣지 네트워크(CloudFront)를 통해 사용자에게서 가장 가까운 곳에서 무제한 고속 전송을 지원합니다.
*   **지능형 자동 확장 (KEDA):** 단순히 서버가 느려지는 것을 기다리지 않고, "대기 번호표가 몇 장 뽑혔는가(Queue Length)"를 감지하여 트래픽 폭주 전에 선제적으로 일꾼(서버)을 복제해냅니다.
*   **가장 실용적인 서버 구조 (Modular Monolith):** 관리하기 어려운 수십 개의 마이크로서비스(MSA)로 무조건 쪼개는 오버엔지니어링 대신, 단일 서버 내에서 핵심 기능들을 폴더(도메인) 단위로 완벽하게 격리하여 초기 스타트업 규모에 맞는 실속과 유지보수성을 모두 챙겼습니다.
