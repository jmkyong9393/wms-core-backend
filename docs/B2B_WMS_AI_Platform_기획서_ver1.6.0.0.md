...
- **WMS Core API Layer (FastAPI):** Python FastAPI 기반 메인 비즈니스 서버. 주문, 출고, 입고, 반품, 재고 증감을 모두 포괄하는 **WMS 코어 트랜잭션 전담**.
  - **회원가입 및 RBAC 권한 제어:** JWT Payload 기반 토큰 인가 체계 도입.
    - **권한 계층:** `MASTER`(관리자/가입승인 권한/풀액세스), `WORKER`(사원/부분 수정 권한), `GUEST`(일반/열람 제한), `PENDING`(가입 대기 상태).
    - 최초 서버 배포 시 DB Seeding을 통해 초기 `MASTER` 계정을 자동 생성하며, 신규 가입자는 `PENDING` 상태로 대기 후 `MASTER`의 승인을 거쳐 활성화됨.
...
```mermaid
flowchart TD
    subgraph "Frontend WebRTC and WASM Edge Pre-processing"
        A[스마트폰 후면 카메라 WebRTC] -->|"1. 디바이스 최대 화질 스트림"| B("Canvas 리사이징 and 압축")
    ...
    subgraph "Backend FastAPI and Celery Redis"
        E -. "Signed Cookie 보안 인증" .-> G[AWS CloudFront Edge Upload]
        G -->|"4. Edge Middleware RBAC and S3 Direct"| H[(PostgreSQL)]
        G -->|"5. Celery Task 발행"| I[Redis Message Broker]
        I -->|"6. 워커 Redis Pub/Sub SSE"| J[Celery Worker]
    end

    subgraph "LangGraph Multi-Agent Pipeline"
        J --> K{Supervisor Agent}
        K <--> V("Vision Agent - GPT4o 결함 BBox 탐지")
    ...
    subgraph "WMS Core"
        ...
        K -->|"8. 불량 판정"| M("WMS 자동 출판사 발주 AutoPO")
    end
    ...
    classDef wms fill:#FFF3E0,stroke:#FB8C00,color:#E65100,stroke-width:2px;
    classDef danger fill:#FFEBEE,stroke:#E53935,color:#B71C1C,stroke-width:2px;
    classDef core fill:#FFFDE7,stroke:#FDD835,color:#F57F17,stroke-width:3px;

    %% =======================
    %% 클래스 적용
    %% =======================
    class A,B,C,E,F frontend;
    class G,H,I,J backend;
    class K,V,P,CriticA,R ai;
    class L,M wms;
    class D danger;
    class K core;
```
...
4. **출고 박스 최적화 (3D Bin Packing):** 다건 주문 시, 도서들의 부피(가로/세로/두께)를 합산하여 가장 물류비가 적게 드는 최적의 택배 박스 사이즈를 작업자에게 자동 추천한다. _(※ 외부 API에서 도서 규격 데이터 누락 시, 카테고리(신국판/B5 등)와 페이지 수를 기반으로 부피/무게를 자동 추정하는 자체 Fallback 알고리즘 탑재)_
...
- **4.1 역량 맞춤형 분업 전략 (7인 체제 - 실명 R&R 기반):**
  ...
  - **Backend Orchestration (서다은):** LangGraph 에이전트와 FastAPI 연동, Redis 기반 실시간 작업 상태 Redis Pub/Sub SSE API 구현, 통합 테스트 수행.
...
- **4.2 6+1주 연구개발 마일스톤:**
  ...
  - **[4~5주차 / 대시보드 및 예외 처리]:** 관리자 대시보드(수동 승인 및 에이전트 로그 조회) 구현, 클라이언트 Redis Pub/Sub SSE 및 재시도(Retry) 로직 적용.
